...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/classes", async (req, res) => {
    ...
    try {
      ...
      console.log(`Fetching classes for ${req.user.role} ${req.user.id}`);
      console.log(`Found ${classes.length} classes for ${req.user.role} ${req.user.id}`);
  ...
  app.post("/api/classes/:id/archive", async (req, res) => {
    if (!req.isAuthenticated() || req.user.role !== "instructor") {
      return res.sendStatus(403);
    }

    await storage.archiveClass(parseInt(req.params.id));
    res.sendStatus(200);
  });

  // Add route to update class description
  ...
  app.get("/api/classes/:classId/assignments", async (req, res) => {
    ...
    try {
      const assignments = await storage.getAssignmentsByClass(parseInt(req.params.classId));
  ...
  app.get("/api/classes/:id/analytics", async (req, res) => {
    try {
      ...
      if (!classInfo || classInfo.instructorId !== user.id) {
      ...
      const studentContracts = await storage.getStudentContractsForClass(classId);
      const contractDistribution = {
        A: studentContracts.filter((sc: any) => sc.contract?.grade === "A").length,
        B: studentContracts.filter((sc: any) => sc.contract?.grade === "B").length,
        C: studentContracts.filter((sc: any) => sc.contract?.grade === "C").length,
        unselected: studentContracts.filter((sc: any) => !sc.contractId).length,
      };
      ...
      for (const assignment of assignments) {
        ...
        const progress = allProgress.filter((p: any) => p.assignmentId === assignment.id);
        
        const completed = progress.filter((p: any) => 
          assignment.scoringType === "status" ? (p.status || 0) >= 3 : (p.numericGrade || 0) >= 3
        ).length;
        const inProgress = progress.filter((p: any) => 
          assignment.scoringType === "status" ? (p.status || 0) === 2 : (p.numericGrade || 0) === 2
        ).length;
        ...
        assignmentCompletion.push({
          ...
          inProgress,
          notStarted,
          completionRate: students.length > 0 ? (completed / students.length) * 100 : 0,
        });
      }
      ...
      for (const student of students) {
        const allProgress = await storage.getStudentProgressForClass(classId);
        const progress = allProgress.filter((p: any) => p.studentId === student.id);
        const studentContract = studentContracts.find((sc: any) => sc.studentId === student.id);
        
        const completedAssignments = progress.filter((p: any) => {
          const assignment = assignments.find(a => a.id === p.assignmentId);
          if (!assignment) return false;
          return assignment.scoringType === "status" 
            ? (p.status || 0) >= 3 
            : (p.numericGrade || 0) >= 3;
        }).length;
        
        const lastActivity = progress.length > 0 
          ? Math.max(...progress.map((p: any) => new Date(p.lastUpdated).getTime()))
          : new Date().getTime();
        ...
        studentProgress.push({
          ...
          contractGrade: (studentContract as any)?.contract?.grade || null,
          isConfirmed: (studentContract as any)?.isConfirmed || false,
      ...
      const averageCompletionRate = studentProgress.length > 0
        ? studentProgress.reduce((sum, sp) => sum + sp.completionRate, 0) / studentProgress.length
        : 0;
      
      const studentsAtRisk = studentProgress.filter(sp => sp.completionRate < 60).length;
      const highPerformers = studentProgress.filter(sp => sp.completionRate >= 90).length;
      const contractConfirmationRate = studentContracts.length > 0
        ? (studentContracts.filter((sc: any) => sc.isConfirmed).length / studentContracts.length) * 100
        : 0;
  ...
  app.patch("/api/classes/:classId/assignments/:assignmentId", async (req, res) => {
    if (!req.isAuthenticated() || req.user.role !== "instructor") {
      return res.sendStatus(403);
    }
  ...
  app.delete("/api/classes/:classId/assignments/:assignmentId", async (req, res) => {
    ...
    try {
      await storage.deleteAssignment(parseInt(req.params.assignmentId));
      res.sendStatus(200);
    } catch (error) {
...
Widener University Contract Grading Portal`
  ...
  app.get("/api/classes/:classId/enrolled-students", async (req, res) => {
    ...
    try {
      ...
      // Only instructors can see enrolled students list
      if (req.user.role !== "instructor" || cls.instructorId !== req.user.id) {
        return res.sendStatus(403);
      }

      const enrolledStudents = await storage.getEnrolledStudents(classId);
